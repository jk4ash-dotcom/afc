...
fun AfcDetailScreen(
    ...
    var prayer by remember(prayerId) { mutableStateOf<AfcPrayer?>(null) }
    var loaded by remember(prayerId) { mutableStateOf(false) }

    LaunchedEffect(prayerId) {
        prayer = withContext(Dispatchers.Default) {
            if (!repository.isPreloaded()) repository.preload()
            repository.afcPrayerById(prayerId)
        }
        loaded = true
    }
